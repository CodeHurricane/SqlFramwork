package com.example.administrator.sqliteframwork.db.update;
...
import com.example.administrator.sqliteframwork.User;
import com.example.administrator.sqliteframwork.UserDao;
import com.example.administrator.sqliteframwork.db.db_core.BaseDbFactory;
import com.example.administrator.sqliteframwork.db.update.xml_element.CreateDb;
import com.example.administrator.sqliteframwork.db.update.xml_element.CreateVersion;
import com.example.administrator.sqliteframwork.db.update.xml_element.UpdateDb;
import com.example.administrator.sqliteframwork.db.update.xml_element.UpdateDbXml;
import com.example.administrator.sqliteframwork.db.update.xml_element.UpdateStep;
...
public class UpdateManager<T> {
    ...
    private static final String BACK_DB="backDb";
    private  File parentFile=new File(Environment.getExternalStorageDirectory(),File_NAME);
    private  File bakFile=new File(parentFile,BACK_DB);
    ...
    public void checkThisVersionTable(Context context )  {
        UserDao userDao= BaseDbFactory.getInstance().getDataHelper(UserDao.class);
    ...
    public void startUpdateDb(Context context)
    {
        ...
        if(getLocalVersionInfo())
        {
            ...
            try {
                ...
                {
                    String logicDbDir=parentFile.getAbsolutePath()+"/update"+"/"+user.getName()+"/logic.db";

                    String  logicCopy=bakFile.getAbsolutePath()+"/"+user.getName()+"/logic.db";
            ...
            if (userList != null && !userList.isEmpty())
            {
                for (User user:userList)
                {
                    String logicDbDir=parentFile.getAbsolutePath()+"/update"+"/"+user.getName()+".db";
        ...
        for (CreateDb cd : createVersion.getCreateDbs())
        {
            ...
            try
            {
                ...
                if (userList != null && !userList.isEmpty())
                {
                    ...
                    {
                        // 获取db
                        sqlitedb = getDb(cd, userList.get(i).getName());
        ...
        for (UpdateDb db : updateDbs)
        {
            ...
            try
            {
                ...
                if (userList != null && !userList.isEmpty())
                {
                    ...
                    for (int i = 0; i < userList.size(); i++)
                    {
                        sqlitedb = getDb(db, userList.get(i).getName());
